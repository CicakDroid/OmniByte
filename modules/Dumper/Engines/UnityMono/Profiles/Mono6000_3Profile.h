#pragma once
// ── Profiles/Mono6000_3Profile.h ──────────────────────────────────
// Unity 6000.3 ships Mono ~6000.3.x (based on .NET 6 runtime).
// Struct layouts derived from mono/mono@mono-6000.3 tag:
//   metadata-internals.h, class-private-definition.h, class-internals.h
// Targets: 64-bit (arm64-v8a), 32-bit (armeabi-v7a).
//
// Core structs stable vs 6000.2. Minor additions in MonoImage for hot reload.
...
class Mono6000_3Profile : public IEngineProfile {
public:
    std::string version() const override { return "6000.3"; }

    uint64_t offsetOf(const std::string& key) const override {
        static const std::unordered_map<std::string, uint64_t> kOffsets = {
            // ── MonoImage (metadata-internals.h) ────────────────────
            {"MonoImageOffset",                 0x0},
            {"MonoImage_sizeof",                0x3B8},
