#pragma once
// ── Profiles/Mono6000_2Profile.h ──────────────────────────────────
// Unity 6000.2 ships Mono ~6000.2.x (based on .NET 6 runtime).
// Struct layouts derived from mono/mono@mono-6000.2 tag:
//   metadata-internals.h, class-private-definition.h, class-internals.h
// Targets: 64-bit (arm64-v8a), 32-bit (armeabi-v7a).
//
// Core structs stable vs 6000.1. Minor additions in MonoImage for hybrid caching.
...
class Mono6000_2Profile : public IEngineProfile {
public:
    std::string version() const override { return "6000.2"; }

    uint64_t offsetOf(const std::string& key) const override {
        static const std::unordered_map<std::string, uint64_t> kOffsets = {
            // ── MonoImage (metadata-internals.h) ────────────────────
            {"MonoImageOffset",                 0x0},
            {"MonoImage_sizeof",                0x3B0},
