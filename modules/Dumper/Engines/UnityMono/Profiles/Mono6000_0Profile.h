...
public:
    ...
    uint64_t offsetOf32(const std::string& key) const override {
        static const std::unordered_map<std::string, uint64_t> kOffsets = {
            // ── MonoImage (32-bit) ─────────────────────────────────
            // MonoImage sizeof/assembly offset varies per-version; core struct
            // fields (MonoClass, MonoClassField, MonoMethod, MonoType) are
            // identical across 6000.x — only MonoImage differs.
            ...
            // ── MonoClass (32-bit, stable across 6000.x) ───────────
