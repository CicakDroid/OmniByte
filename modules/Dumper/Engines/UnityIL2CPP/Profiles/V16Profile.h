#pragma once
// ── Profiles/V16Profile.h ───────────────────────────────────────
// IL2CPP metadata version 16 (Unity 5.3.0–5.4.0).
// Source: Perfare/Il2CppDumper @ master (commit 4741d46), Il2Cpp/MetadataClass.cs
// Header: Base fields only (no version-gated fields for V16).
#include "../../../DumperCore/IEngineProfile.h"
#include <cstddef>
#include <cstdint>
#include <string>
#include <unordered_map>

namespace omnibyte::dumper::unityil2cpp {

class V16Profile : public IEngineProfile {
public:
    std::string version() const override { return "16"; }

    uint64_t offsetOf(const std::string& key) const override {
        // V16 header: base fields only — no fieldRefs (Min=19),
        // no metadataUsage (Min=19), no referencedAssemblies (Min=20),
        // no attributes/unresolvedVCall/WinRT (Min=21+).
        // src: Il2CppDumper/Il2Cpp/MetadataClass.cs @ master
        static const std::unordered_map<std::string, uint64_t> kOffsets = {
            {"sanity",                            0x00},
            {"version",                           0x04},
            {"stringLiteralOffset",               0x08},
            {"stringLiteralSize",                 0x0C},
            {"stringLiteralDataOffset",           0x10},
            {"stringLiteralDataSize",             0x14},
            {"stringOffset",                      0x18},
            {"stringSize",                        0x1C},
            {"eventsOffset",                      0x20},
            {"eventsSize",                        0x24},
            {"propertiesOffset",                  0x28},
            {"propertiesSize",                    0x2C},
            {"methodsOffset",                     0x30},
            {"methodsSize",                       0x34},
            {"parameterDefaultValuesOffset",      0x38},
            {"parameterDefaultValuesSize",        0x3C},
            {"fieldDefaultValuesOffset",          0x40},
            {"fieldDefaultValuesSize",            0x44},
            {"fieldAndParameterDefaultValueDataOffset", 0x48},
            {"fieldAndParameterDefaultValueDataSize",   0x4C},
            {"fieldMarshaledSizesOffset",         0x50},
            {"fieldMarshaledSizesSize",           0x54},
            {"parametersOffset",                  0x58},
            {"parametersSize",                    0x5C},
            {"fieldsOffset",                      0x60},
            {"fieldsSize",                        0x64},
            {"genericParametersOffset",           0x68},
            {"genericParametersSize",             0x6C},
            {"genericParameterConstraintsOffset", 0x70},
            {"genericParameterConstraintsSize",   0x74},
            {"genericContainersOffset",           0x78},
            {"genericContainersSize",             0x7C},
            {"nestedTypesOffset",                 0x80},
            {"nestedTypesSize",                   0x84},
            {"interfacesOffset",                  0x88},
            {"interfacesSize",                    0x8C},
            {"vtableMethodsOffset",               0x90},
            {"vtableMethodsSize",                 0x94},
            {"interfaceOffsetsOffset",            0x98},
            {"interfaceOffsetsSize",              0x9C},
            {"typeDefinitionsOffset",             0xA0},
            {"typeDefinitionsSize",               0xA4},
            // No rgctx for V16 — but rgctxEntries is Max=24.1, so present.
            // However, header field ordering in Il2CppDumper places
            // images/assemblies AFTER rgctx for Max=24.1.
            // V16 has rgctx (Max=24.1): rgctxEntriesOffset/Count
            {"rgctxEntriesOffset",                0xA8},
            {"rgctxEntriesCount",                 0xAC},
            {"imagesOffset",                      0xB0},
            {"imagesSize",                        0xB4},
            {"assembliesOffset",                  0xB8},
            {"assembliesSize",                    0xBC},
        };
        auto it = kOffsets.find(key);
        return it != kOffsets.end() ? it->second : 0;
    }

    size_t structSize(const std::string& key) const override {
        // V16 structs: has customAttributeIndex (Max=24), no token (Min=19)
        // delegateWrapper/marshalingFunctions present (Max=22, V16 <= 22)
        // No ccwFunction/guid (Min=21, V16 < 21)
        // rgctxStartIndex/Count present (Max=24.1, V16 <= 24.1)
        // src: Il2CppDumper/Il2Cpp/MetadataClass.cs @ master
        static const std::unordered_map<std::string, size_t> kSizes = {
            {"Il2CppTypeDefinition",  0x78}, // has customAttributeIndex + delegateWrapper + marshaling + rgctx, no token
            {"Il2CppMethodDefinition", 0x34}, // has customAttributeIndex + methodIndex/invoker/delegate/rgctx
            {"Il2CppFieldDefinition",  0x0C}, // has customAttributeIndex, no token (Min=19)
            {"Il2CppParameterDefinition", 0x10}, // has customAttributeIndex
            {"Il2CppPropertyDefinition", 0x14}, // has customAttributeIndex, no token (Min=19)
            {"Il2CppEventDefinition",   0x18}, // has customAttributeIndex, no token (Min=19)
            {"Il2CppImageDefinition",   0x14}, // no token (Min=19), no exportedType (Min=24), no customAttributeStart (Min=24.1)
            {"Il2CppAssemblyDefinition", 0x3C}, // has customAttributeIndex, no referencedAssembly (Min=20)
            {"Il2CppAssemblyNameDefinition", 0x34}, // has hashValueIndex (Max=24.3)
            {"Il2CppCustomAttributeTypeRange", 0x08}, // no token (Min=24.1)
        };
        auto it = kSizes.find(key);
        return it != kSizes.end() ? it->second : 0;
    }

    bool validate(const uint8_t* headerBytes, size_t len) const override {
        // src: Il2CppDumper/Il2Cpp/Metadata.cs @ master
        if (len < 8) return false;
        uint32_t magic = headerBytes[0] | (headerBytes[1] << 8) |
                         (headerBytes[2] << 16) | (headerBytes[3] << 24);
        int32_t ver = headerBytes[4] | (headerBytes[5] << 8) |
                      (headerBytes[6] << 16) | ((uint32_t)headerBytes[7] << 24);
        return magic == 0xFAB11BAF && ver == 16;
    }
};

} // namespace omnibyte::dumper::unityil2cpp
