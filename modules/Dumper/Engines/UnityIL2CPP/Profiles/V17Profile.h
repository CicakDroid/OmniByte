#pragma once
// ── Profiles/V17Profile.h ───────────────────────────────────────
// IL2CPP metadata version 17 (Unity 5.5.0–5.6.0).
// Source: Perfare/Il2CppDumper @ master, Il2Cpp/MetadataClass.cs
// Header: Same as V16 — no version-gated fields differ between 16 and 17.
...
class V17Profile : public IEngineProfile {
public:
    std::string version() const override { return "17"; }

    uint64_t offsetOf(const std::string& key) const override {
        // V17 header: identical layout to V16 — no version-gated additions/removals
        // at versions 17. All [Version] attributes use Min=19+ or Min=20+, etc.
        ...
        static const std::unordered_map<std::string, uint64_t> kOffsets = {
            ...
            // rgctxEntries (Max=24.1, present for V17 since 17 <= 24.1)
    ...
    size_t structSize(const std::string& key) const override {
        // V17 structs: identical to V16 — same version-gated fields apply
        // has customAttributeIndex (Max=24), no token (Min=19)
        // delegateWrapper/marshalingFunctions present (Max=22, V17 <= 22)
        // No ccwFunction/guid (Min=21, V17 < 21)
        // rgctxStartIndex/Count present (Max=24.1, V17 <= 24.1)
        // src: Il2CppDumper/Il2Cpp/MetadataClass.cs @ master (4741d46)
        static const std::unordered_map<std::string, size_t> kSizes = {
            ...
            {"Il2CppImageDefinition",   0x14}, // no token (Min=19), no exportedType (Min=24)
    ...
    bool validate(const uint8_t* headerBytes, size_t len) const override {
        ...
        return magic == 0xFAB11BAF && ver == 17;
    }
};
