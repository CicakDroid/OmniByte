#pragma once
// ── Profiles/V21Profile.h ───────────────────────────────────────
// IL2CPP metadata version 21 (Unity 2018.1–2018.2).
// Source: Perfare/Il2CppDumper @ master, Il2Cpp/MetadataClass.cs
// Header: V20 + attributesInfo/attributeTypes (Min=21)
...
class V21Profile : public IEngineProfile {
public:
    std::string version() const override { return "21"; }

    uint64_t offsetOf(const std::string& key) const override {
        // V21 header: V20 + attributesInfo/attributeTypes (Min=21)
        ...
        static const std::unordered_map<std::string, uint64_t> kOffsets = {
            ...
            // rgctxEntries (Max=24.1, present for V21 since 21 <= 24.1)
            ...
            {"assembliesSize",                    0xBC},
            // metadataUsage (Min=19, present for V21 since 21 >= 19)
            {"metadataUsageListsOffset",          0xC0},
            {"metadataUsageListsCount",           0xC4},
            {"metadataUsagePairsOffset",          0xC8},
            {"metadataUsagePairsCount",           0xCC},
            // fieldRefs (Min=19, present for V21 since 21 >= 19)
            {"fieldRefsOffset",                   0xD0},
            {"fieldRefsSize",                     0xD4},
            // referencedAssemblies (Min=20, present for V21 since 21 >= 20)
            {"referencedAssembliesOffset",        0xD8},
            {"referencedAssembliesSize",          0xDC},
            // attributesInfo (Min=21, present for V21 since 21 >= 21)
    ...
    size_t structSize(const std::string& key) const override {
        // V21 structs: has customAttributeIndex (Max=24), rgctx (Min=21, Max=24.1)
        // delegateWrapper/marshalingFunctions present (Max=22, V21 <= 22)
        // ccwFunction/guid present (Min=21, Max=22, V21 <= 22)
        // src: Il2CppDumper/Il2Cpp/MetadataClass.cs @ master (4741d46)
        static const std::unordered_map<std::string, size_t> kSizes = {
            {"Il2CppTypeDefinition",  0x88}, // has customAttributeIndex + rgctx + delegateWrapper + marshaling + ccw + guid
    ...
    bool validate(const uint8_t* headerBytes, size_t len) const override {
        ...
        return magic == 0xFAB11BAF && ver == 21;
    }
};
