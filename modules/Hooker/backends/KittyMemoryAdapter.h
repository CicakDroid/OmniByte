#pragma once
// KittyMemory adapter — IHookBackend for KittyMemory (MJx0).
// Source: https://github.com/MJx0/KittyMemory (MIT)
// ARM inline hook + memory patch framework.

#include "../IHookBackend.h"

namespace omnibyte::runtime::backends {

class KittyMemoryAdapter : public IHookBackend {
public:
    std::string name() const override { return "KittyMemory"; }
    bool isAvailable() const override { return true; }
    bool hookFunction(uintptr_t addr, void* replacement, void** originalOut) override;
    bool unhook(uintptr_t addr) override;
    bool patchMemory(uintptr_t addr, const uint8_t* data, size_t size) override;
};

} // namespace omnibyte::runtime::backends
