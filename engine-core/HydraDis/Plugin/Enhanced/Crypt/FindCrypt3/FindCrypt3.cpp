// FindCrypt3 — Crypto constant finder.
// Source: https://github.com/HongThatCong/FindCrypt3
// Commit: master branch, 2026-09-01
// License: MIT
// IDA plugin ported as standalone crypto constant scanner.

#include "FindCrypt3.h"

#include <cstring>
#include <fstream>

namespace omnibyte::deob {

// Known crypto constants
static const uint8_t AES_SBOX[256] = {
    ,
};

static const uint8_t SHA256_H0[32] = {
    0x67, 0xE6, 0x09, 0x6A, 0xAB, 0xD6, 0xCA, 0x3C,
    0x6A, 0x09, 0xE6, 0x67, 0xBB, 0x67, 0xAE, 0x85,
    0x3C, 0x6E, 0xF3, 0x72, 0xA5, 0x4F, 0xF5, 0x3A,
    0x51, 0x0E, 0x52, 0x7F, 0x9B, 0x05, 0x68, 0x8C,
};

static const uint8_t DES_SBOX1[64] = {
    0x0E, 0x04, 0x0D, 0x01, 0x02, 0x0F, 0x0B, 0x08,
    0x03, 0x0A, 0x06, 0x0C, 0x05, 0x09, 0x00, 0x07,
    0x00, 0x0F, 0x07, 0x04, 0x0E, 0x02, 0x0D, 0x01,
    0x0A, 0x06, 0x0C, 0x0B, 0x09, 0x05, 0x03, 0x08,
    0x04, 0x01, 0x0E, 0x08, 0x0D, 0x06, 0x02, 0x0B,
    0x0F, 0x0C, 0x09, 0x07, 0x03, 0x0A, 0x05, 0x00,
    0x0F, 0x0C, 0x08, 0x02, 0x04, 0x09, 0x01, 0x07,
    0x05, 0x0B, 0x03, 0x0E, 0x0A, 0x00, 0x06, 0x0D,
};

static const uint32_t MD5_T[64] = {
    0xd76aa478, 0xe8c7b756, 0x242070db, 0xc1bdceee,
    0xf57c0faf, 0x4787c62a, 0xa8304613, 0xfd469501,
    0x698098d8, 0x8b44f7af, 0xffff5bb1, 0x895cd7be,
    0x6b901122, 0xfd987193, 0xa679438e, 0x49b40821,
};

// XXTEA/TEA magic constant (delta = floor(2^32 / golden_ratio))
static const uint32_t XXTEA_DELTA = 0x9E3779B9;
static const uint32_t XXTEA_DELTA_ALT = 0x61C88647;

// RC4 KSA loop bound patterns (cmp reg, 0x100)
static const uint8_t RC4_PAT_1[] = {0x3D, 0x00, 0x01, 0x00, 0x00};           // cmp eax, 0x100
static const uint8_t RC4_PAT_2[] = {0x81, 0xF8, 0x00, 0x01, 0x00, 0x00};    // cmp eax, 0x100 (modrm)
static const uint8_t RC4_PAT_3[] = {0x81, 0xF9, 0x00, 0x01, 0x00, 0x00};    // cmp ecx, 0x100
static const uint8_t RC4_PAT_4[] = {0x81, 0xFA, 0x00, 0x01, 0x00, 0x00};    // cmp edx, 0x100
static const uint8_t RC4_PAT_5[] = {0x81, 0xFB, 0x00, 0x01, 0x00, 0x00};    // cmp ebx, 0x100
static const uint8_t RC4_PAT_6[] = {0x48, 0x3D, 0x00, 0x01, 0x00, 0x00};    // cmp rax, 0x100
static const uint8_t RC4_PAT_7[] = {0x48, 0x81, 0xF8, 0x00, 0x01, 0x00, 0x00}; // cmp rax, 0x100 (modrm)

// Whirlpool S-box (first 32 bytes)
static const uint8_t WHIRLPOOL_SBOX[32] = {
    0x18, 0x23, 0xC6, 0xE8, 0x87, 0xB8, 0x01, 0x4F,
    0x03, 0xD6, 0x34, 0x1E, 0x57, 0x72, 0x7A, 0xC0,
    0xAC, 0x61, 0x35, 0x44, 0x3C, 0x22, 0x64, 0x19,
    0xD3, 0x8B, 0x86, 0xC2, 0x9F, 0x08, 0xD5, 0xF7,
};

// RIPEMD-160 H0 initial values
static const uint8_t RIPEMD160_H0[20] = {
    0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF,
    0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10,
    0xF0, 0xE1, 0xD2, 0xC3,
};

// Camellia S-box (first 32 bytes)
static const uint8_t CAMELLIA_SBOX[32] = {
    0x70, 0x82, 0x2C, 0xEC, 0xB3, 0x27, 0xC0, 0xE5,
    0x12, 0xA5, 0x1A, 0x0B, 0xA2, 0x0A, 0x14, 0xA8,
    0x46, 0x06, 0x42, 0x3A, 0x08, 0x24, 0x16, 0x1E,
    0xBA, 0xE0, 0x56, 0x22, 0x68, 0xC5, 0x73, 0x18,
};

// Serpent S-box (first 32 bytes from S0)
static const uint8_t SERPENT_SBOX[32] = {
    0x33, 0x32, 0x30, 0x31, 0x38, 0x39, 0x3B, 0x3A,
    0x37, 0x36, 0x34, 0x35, 0x3E, 0x3F, 0x3D, 0x3C,
    0x43, 0x42, 0x40, 0x41, 0x48, 0x49, 0x4B, 0x4A,
    0x47, 0x46, 0x44, 0x45, 0x4E, 0x4F, 0x4D, 0x4C,
};

// Twofish P-box (first 32 bytes from P0)
static const uint8_t TWOFISH_PBOX[32] = {
    0xA9, 0x67, 0xB4, 0xD5, 0x2C, 0x6E, 0x31, 0x07,
    0x40, 0x28, 0x5E, 0x3D, 0x65, 0xB8, 0x04, 0x8C,
    0x96, 0x43, 0x6A, 0x32, 0x1C, 0x0C, 0x27, 0x53,
    0x92, 0xCF, 0xE1, 0x4B, 0x7D, 0x39, 0x64, 0x5D,
};

// GOST S-box (first 16 bytes from S1)
static const uint8_t GOST_SBOX[16] = {
    0x09, 0x06, 0x03, 0x02, 0x08, 0x0B, 0x01, 0x07,
    0x0A, 0x04, 0x0E, 0x0F, 0x0C, 0x00, 0x0D, 0x05,
};

// RC2 S-box (first 16 bytes)
static const uint8_t RC2_SBOX[16] = {
    0xD9, 0x78, 0xF9, 0xC4, 0x1D, 0x93, 0xEA, 0xDB,
    0x7F, 0x2E, 0x30, 0xF2, 0xEC, 0x6D, 0x51, 0x5B,
};

// ChaCha/Salsa20 constants "expand 32-byte k"
static const uint8_t CHACHA_CONSTANTS[16] = {
    0x65, 0x78, 0x70, 0x61, 0x6E, 0x64, 0x20, 0x33,
    0x32, 0x2D, 0x62, 0x79, 0x74, 0x65, 0x20, 0x6B,
};

// === P5: Hash Functions ===

// SHA-1 H0 initial hash values (20 bytes)
// Source: FIPS 180-4 Section 5.3.1
static const uint8_t SHA1_H0[20] = {
    0x67, 0x45, 0x23, 0x01, 0xEF, 0xCD, 0xAB, 0x89,
    0x98, 0xBA, 0xDC, 0xFE, 0x10, 0x32, 0x54, 0x76,
    0xC3, 0xD2, 0xE1, 0xF0,
};

// SHA-384 H0 initial hash values (48 bytes)
// Source: FIPS 180-4 Section 5.3.4
static const uint8_t SHA384_H0[48] = {
    0xCB, 0xBB, 0x9D, 0x5D, 0xC1, 0x05, 0x9E, 0xD8,
    0x62, 0x9A, 0x29, 0x2A, 0x36, 0x7C, 0xD5, 0x07,
    0x91, 0x59, 0x01, 0x5A, 0x30, 0x70, 0xDD, 0x17,
    0x15, 0x2F, 0xEC, 0xD8, 0xF7, 0x0E, 0x59, 0x39,
    0x67, 0x33, 0x26, 0x67, 0xFF, 0xC0, 0x0B, 0x31,
    0x8E, 0xB4, 0x4A, 0x87, 0x68, 0x58, 0x15, 0x11,
};

// SHA-512 H0 initial hash values (64 bytes)
// Source: FIPS 180-4 Section 5.3.4
static const uint8_t SHA512_H0[64] = {
    0x6A, 0x09, 0xE6, 0x67, 0xF3, 0xBC, 0xC9, 0x08,
    0xBB, 0x67, 0xAE, 0x85, 0x84, 0xCA, 0xA7, 0x3B,
    0x3C, 0x6E, 0xF3, 0x72, 0xFE, 0x94, 0xF8, 0x2B,
    0xA5, 0x4F, 0xF5, 0x3A, 0x5F, 0x1D, 0x36, 0xF1,
    0x51, 0x0E, 0x52, 0x7F, 0xAD, 0xE6, 0x82, 0xD1,
    0x9B, 0x05, 0x68, 0x8C, 0x2B, 0x3E, 0x6C, 0x1F,
    0x1F, 0x83, 0xD9, 0xAB, 0xFB, 0x41, 0xBD, 0x6B,
    0x5B, 0xE0, 0xCD, 0x19, 0x13, 0x7E, 0x21, 0x79,
};

// MD2 PI_SUBST S-box (256 bytes)
// Source: RFC 1319
static const uint8_t MD2_PI_SUBST[256] = {
    ,
};

// MD4 T[1..16] constants (64 bytes)
// Source: RFC 1320
static const uint8_t MD4_T16[64] = {
    0xd7, 0x6a, 0xa4, 0x78, 0xe8, 0xc7, 0xb7, 0x56,
    0x24, 0x20, 0x70, 0xdb, 0xc1, 0xbd, 0xce, 0xee,
    0xf5, 0x7c, 0x0f, 0xaf, 0x47, 0x87, 0xc6, 0x2a,
    0xa8, 0x30, 0x46, 0x13, 0xfd, 0x46, 0x95, 0x01,
    0x69, 0x80, 0x98, 0xd8, 0x8b, 0x44, 0xf7, 0xaf,
    0xff, 0xff, 0x5b, 0xb1, 0x89, 0x5c, 0xd7, 0xbe,
    0x6b, 0x90, 0x11, 0x22, 0xfd, 0x98, 0x71, 0x93,
    0xa6, 0x79, 0x43, 0x8e, 0x49, 0xb4, 0x08, 0x21,
};

// === P6: Block Ciphers ===

// SEED KC constants (64 bytes)
// Source: ISO/IEC 18033-3, KISA SEED specification
static const uint8_t SEED_KC[64] = {
    0xB9, 0x79, 0x37, 0x9E, 0x73, 0xF3, 0x6E, 0x3C,
    0xE6, 0xE6, 0xDD, 0x78, 0xCC, 0xCD, 0xBB, 0xF1,
    0x99, 0x9B, 0x77, 0xE3, 0x33, 0x37, 0xEF, 0xC6,
    0x67, 0x6E, 0xDE, 0x8D, 0xCF, 0xDC, 0xBC, 0x1B,
    0x9E, 0xB9, 0x79, 0x37, 0x3C, 0x73, 0xF3, 0x6E,
    0x78, 0xE6, 0xE6, 0xDD, 0xF1, 0xCC, 0xCD, 0xBB,
    0xE3, 0x99, 0x9B, 0x77, 0xC6, 0x33, 0x37, 0xEF,
    0x8D, 0x67, 0x6E, 0xDE, 0x1B, 0xCF, 0xDC, 0xBC,
};

// LEA Delta constant (32 bytes)
// Source: Korean LEA standard, https://github.com/JeongHan-Bae/LEA-C
static const uint8_t LEA_DELTA[32] = {
    0xC3, 0xEF, 0xE9, 0xDB, 0x44, 0x62, 0x6B, 0x02,
    0x79, 0xE2, 0x7C, 0x8A, 0x78, 0xDF, 0x30, 0xEC,
    0x71, 0x5E, 0xA4, 0x9E, 0xC7, 0x85, 0xDA, 0x0A,
    0xE0, 0x4E, 0xF2, 0x2A, 0xE5, 0xC4, 0x09, 0x57,
};

// === P7: Extended Hash/MAC ===

// Serpent full S-boxes S0-S7 (128 bytes)
// Source: Serpent specification
static const uint8_t SERPENT_SBOX_FULL[128] = {
    ,
};

// Tiger IV (24 bytes)
// Source: Tiger hash specification
static const uint8_t TIGER_IV[24] = {
    0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF,
    0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10,
    0xF0, 0x96, 0xA5, 0xB4, 0xC3, 0xB2, 0xE1, 0x87,
};

// HAVAL IV (32 bytes)
// Source: https://github.com/php/php-src/blob/fe52e5b6/ext/hash/hash_haval.c
static const uint8_t HAVAL_IV[32] = {
    0x24, 0x3F, 0x6A, 0x88, 0x85, 0xA3, 0x08, 0xD3,
    0x13, 0x19, 0x8A, 0x2E, 0x03, 0x70, 0x73, 0x44,
    0xA4, 0x09, 0x38, 0x22, 0x29, 0x9F, 0x31, 0xD0,
    0x08, 0x2E, 0xFA, 0x98, 0xEC, 0x4E, 0x6C, 0x89,
};

// BLAKE2s IV (32 bytes)
// Source: RFC 7693
static const uint8_t BLAKE2S_IV[32] = {
    0x6A, 0x09, 0x66, 0x7E, 0xBB, 0x67, 0xAE, 0x85,
    0x3C, 0x6E, 0xF3, 0x72, 0xA5, 0x4F, 0xF5, 0x3A,
    0x51, 0x0E, 0x52, 0x7F, 0x9B, 0x05, 0x68, 0x8C,
    0x1F, 0x83, 0xD9, 0xAB, 0x5B, 0xE0, 0xCD, 0x19,
};

// Keccak round constants (192 bytes = 24 x 8 bytes)
// Source: Keccak team, NIST SHA-3
static const uint8_t KECCAK_RC[192] = {
    ,
};

// === P9: Modern Ciphers ===

// SIMON-64/128 z3 constant (8 bytes)
// Source: NSA Simon/Speck paper, bit-reversed for implementation
static const uint8_t SIMON64_128_Z3[8] = {
    0xDB, 0xAC, 0x65, 0x3E, 0x00, 0x48, 0xA7, 0x34,
};

// Skipjack F-table (256 bytes)
// Source: NSA Skipjack specification
static const uint8_t SKIPJACK_FTABLE[256] = {
    ,
};

// Square S-box prefix (32 bytes)
// Source: Square cipher specification
static const uint8_t SQUARE_SBOX_PREFIX[32] = {
    0xBB, 0x63, 0x2E, 0x55, 0x52, 0x44, 0x0C, 0x5E,
    0x64, 0x41, 0x32, 0x56, 0x60, 0x04, 0x34, 0x1F,
    0x08, 0x51, 0x23, 0x31, 0x35, 0x6B, 0x1C, 0x48,
    0x58, 0x0D, 0x3A, 0x28, 0x47, 0x18, 0x37, 0x01,
};

// SHARK S-box prefix (32 bytes)
// Source: Crypto++ sharkbox.cpp
static const uint8_t SHARK_SBOX_PREFIX[32] = {
    0x63, 0x5A, 0x8E, 0x47, 0x24, 0xB0, 0x89, 0x6E,
    0xC7, 0x52, 0x10, 0xD6, 0xB3, 0x14, 0xC0, 0x86,
    0x0A, 0xA4, 0xF5, 0x1B, 0x62, 0x5E, 0xC2, 0x8D,
    0x91, 0x4F, 0x03, 0xE9, 0x3D, 0x21, 0xA5, 0xF8,
};

// Curve25519-donna field prime 2^255-19 (32 bytes)
// Source: https://github.com/agl/curve25519-donna
static const uint8_t DONNA_FIELD_PRIME[32] = {
    0xED, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x7F,
};

// === P11: Tier 1 Algorithms ===

// MD5MAC T constants (48 bytes)
// Source: FindCrypt-Ghidra database
static const uint8_t MD5MAC_T[48] = {
    0x97, 0xEF, 0x45, 0xAC, 0x29, 0x0F, 0x43, 0xCD,
    0x45, 0x7E, 0x1B, 0x55, 0x1C, 0x80, 0x11, 0x34,
    0xB1, 0x77, 0xCE, 0x96, 0x2E, 0x72, 0x8E, 0x7C,
    0x5F, 0x5A, 0xAB, 0x0A, 0x36, 0x43, 0xBE, 0x18,
    0x9D, 0x21, 0xB4, 0x21, 0xBC, 0x87, 0xB9, 0x4D,
    0xA2, 0x9D, 0x27, 0xBD, 0xC7, 0x5B, 0xD7, 0xC3,
};

// SHACAL2 K constants (256 bytes)
// Source: FindCrypt-Ghidra database
static const uint8_t SHACAL2_K[256] = {
    ,
};

// WAKE T-table fixed values (40 bytes)
// Source: FindCrypt-Ghidra database
static const uint8_t WAKE_TT[40] = {
    0x3B, 0x8F, 0x6A, 0x72, 0x5C, 0x3B, 0x9A, 0xE6,
    0xE5, 0x1F, 0xC7, 0xD3, 0xD2, 0x73, 0x3C, 0xAB,
    0xB3, 0x8E, 0x3A, 0x4D, 0xE8, 0xD6, 0x96, 0x03,
    0x7A, 0x2F, 0x4C, 0x3D, 0xF3, 0x7C, 0xE2, 0x9E,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};

// Curve25519 modular multiplication constants (40 bytes each)
// Source: FindCrypt-Ghidra database
static const uint8_t MODM_M_64[40] = {
    0xED, 0xD3, 0xF5, 0x5C, 0x1A, 0x63, 0x12, 0x00,
    0x58, 0xD6, 0x9C, 0xF7, 0xA2, 0xDE, 0xF9, 0x00,
    0xDE, 0x14, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00, 0x00,
};

static const uint8_t MODM_MU_64[40] = {
    0x1B, 0x13, 0x2C, 0x0A, 0xA3, 0xE5, 0x9C, 0x00,
    0xED, 0xA7, 0x29, 0x63, 0x08, 0x5D, 0x21, 0x00,
    0x06, 0x21, 0xEB, 0xFF, 0xFF, 0xFF, 0x00, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0xFF,
    0xFF, 0xFF, 0x0F, 0x00, 0x00, 0x00, 0x00, 0x00,
};

// === P12: Tier 2 Algorithms ===

// HIGHT Delta constants (128 bytes)
// Source: FindCrypt-Ghidra database, Korean HIGHT standard
static const uint8_t HIGHT_DELTA[128] = {
    ,
};

// SAFER EXP table prefix (32 bytes from 256-byte table)
// Source: FindCrypt-Ghidra database, SAFER-SK64/SK128
static const uint8_t SAFER_EXP_PREFIX[32] = {
    0x01, 0x2D, 0xE2, 0x93, 0xBE, 0x45, 0x15, 0xAE,
    0x78, 0x03, 0x87, 0xA4, 0xB8, 0x38, 0xCF, 0x3F,
    0x08, 0x67, 0x09, 0x94, 0xEB, 0x26, 0xA8, 0x6B,
    0xBD, 0x18, 0x34, 0x1B, 0xBB, 0xBF, 0x72, 0xF7,
};

// CAST-256 sigma table prefix (32 bytes from 768-byte table)
// Source: FindCrypt-Ghidra database, CAST-256
static const uint8_t CAST256_SIGMA_PREFIX[32] = {
    0x99, 0x79, 0x82, 0x5A, 0xA1, 0xD6, 0x51, 0xD1,
    0xA9, 0x33, 0x21, 0x48, 0xB1, 0x90, 0xF0, 0xBE,
    0xB9, 0xED, 0xBF, 0x35, 0xC1, 0x4A, 0x8F, 0xAC,
    0xC9, 0xA7, 0x5E, 0x23, 0xD1, 0x04, 0x2E, 0x9A,
};

// BLAKE2b SIGMA permutation table (192 bytes = 12 x 16 bytes)
// Source: RFC 7693
static const uint8_t BLAKE2B_SIGMA[192] = {
    ,
};

// --- Tier 3: CAST-128, MARS, Rijndael Te0, RawDES Spbox (prefix arrays) ---

// CAST-128 S-box prefix (64 bytes)
static const uint8_t CAST128_S_PREFIX[64] = {
    0xD4, 0x40, 0xFB, 0x30, 0x0B, 0xFF, 0xA0, 0x9F,
    0x2F, 0xCD, 0xEC, 0x6B, 0x7A, 0x8C, 0x25, 0x3F,
    0x2F, 0x3F, 0x21, 0x1E, 0xD3, 0x4D, 0x00, 0x9C,
    0x40, 0xE5, 0x03, 0x60, 0x49, 0xC9, 0x9F, 0xCF,
    0x27, 0xAF, 0xD4, 0xBF, 0xB5, 0xBD, 0xB8, 0x89,
    0x04, 0x00, 0x3E, 0x27, 0x59, 0x6D, 0x09, 0x8E,
    0x0A, 0x06, 0x36, 0xED, 0x26, 0x1C, 0x31, 0x51,
    0xD6, 0x6E, 0x7C, 0x28, 0xEF, 0xFD, 0x42, 0x20,
};

// MARS S-box prefix (64 bytes)
static const uint8_t MARS_SBOX_PREFIX[64] = {
    0x79, 0xC4, 0xD0, 0x09, 0xE0, 0xFF, 0xC8, 0x28,
    0x39, 0x6C, 0xAA, 0x84, 0x87, 0x72, 0xAD, 0x9D,
    0xE3, 0x9B, 0xFF, 0x7D, 0x61, 0x83, 0x26, 0xD4,
    0xD4, 0xA1, 0x6D, 0xC9, 0x93, 0xCC, 0x74, 0x79,
    0x2E, 0x58, 0xD0, 0x85, 0x05, 0x57, 0x4B, 0x2A,
    0x62, 0x6A, 0xA1, 0x1C, 0x9D, 0x27, 0xBD, 0xC3,
    0xE5, 0x25, 0x1F, 0x0F, 0x2F, 0x37, 0x60, 0x51,
    0xFB, 0xC1, 0x95, 0xC6, 0xE4, 0xF1, 0x7F, 0x4D,
};

// Rijndael Te0 prefix (64 bytes)
static const uint8_t RIJNDAEL_TE0_PREFIX[64] = {
    0xA5, 0x63, 0x63, 0xC6, 0x84, 0x7C, 0x7C, 0xF8,
    0x99, 0x77, 0x77, 0xEE, 0x8D, 0x7B, 0x7B, 0xF6,
    0x0D, 0xF2, 0xF2, 0xFF, 0xBD, 0x6B, 0x6B, 0xD6,
    0xB1, 0x6F, 0x6F, 0xDE, 0x54, 0xC5, 0xC5, 0x91,
    0x50, 0x03, 0x03, 0x06, 0x00, 0x30, 0x10, 0x10,
    0x2A, 0x96, 0x67, 0x6C, 0xE9, 0x7D, 0x2B, 0x2B,
    0x56, 0x19, 0xFE, 0xFE, 0xE7, 0x62, 0xD7, 0xD7,
    0xB5, 0xE6, 0xAB, 0xAB, 0x4D, 0x9A, 0x76, 0x76,
};

// RawDES Spbox prefix (64 bytes)
static const uint8_t RAWDES_SPBOX_PREFIX[64] = {
    0x00, 0x04, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x01, 0x00, 0x04, 0x04, 0x01, 0x01,
    0x04, 0x00, 0x01, 0x01, 0x04, 0x04, 0x01, 0x00,
    0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00,
    0x00, 0x04, 0x00, 0x00, 0x00, 0x04, 0x01, 0x01,
    0x04, 0x04, 0x01, 0x01, 0x00, 0x04, 0x00, 0x00,
    0x04, 0x04, 0x00, 0x01, 0x04, 0x00, 0x01, 0x01,
    0x00, 0x00, 0x00, 0x01, 0x04, 0x00, 0x00, 0x00,
};

std::vector<CryptoHit> FindCrypt3Engine::scanRegion(const uint8_t* data, size_t size) const {
    std::vector<CryptoHit> hits;

    for (size_t i = 0; i + 4 <= size; i += 4) {
        if (checkXxtea(data, size, i)) {
            uint32_t val = *reinterpret_cast<const uint32_t*>(data + i);
            double conf = (val == XXTEA_DELTA) ? 0.85 : 0.70;
            hits.push_back({static_cast<uintptr_t>(i), "XXTEA", conf, "TEA/XXTEA DELTA constant"});
        }
    }

    for (size_t i = 0; i + 7 <= size; ++i) {
        if (checkRc4Ksa(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "RC4", 0.75, "RC4 KSA loop bound (cmp reg, 0x100)"});
        }
    }

    for (size_t i = 0; i + 16 <= size; ++i) {
        if (checkRc2(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "RC2", 0.80, "RC2 S-box"});
        }
    }

    for (size_t i = 0; i + 16 <= size; ++i) {
        if (checkGost(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "GOST", 0.80, "GOST S-box"});
        }
    }

    for (size_t i = 0; i + 40 <= size; ++i) {
        if (checkWake(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "WAKE", 0.80, "WAKE T-table"});
        }
    }

    for (size_t i = 0; i + 256 <= size; ++i) {
        if (checkAesSbox(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "AES", 0.95, "AES S-box (256 bytes)"});
        }
    }

    for (size_t i = 0; i + 64 <= size; ++i) {
        if (checkDesSbox(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "DES", 0.85, "DES S-box (64 bytes)"});
        }
    }

    for (size_t i = 0; i + 64 <= size; ++i) {
        if (checkRawDesSpbox(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "RawDES-Spbox", 0.80, "RawDES S/p-boxes"});
        }
    }

    for (size_t i = 0; i + 256 <= size; ++i) {
        if (checkBlowfish(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "Blowfish", 0.80, "Blowfish P-box"});
        }
    }

    for (size_t i = 0; i + 32 <= size; ++i) {
        if (checkCamellia(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "Camellia", 0.85, "Camellia S-box"});
        }
    }

    for (size_t i = 0; i + 32 <= size; ++i) {
        if (checkSerpent(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "Serpent", 0.80, "Serpent S-box"});
        }
    }

    for (size_t i = 0; i + 128 <= size; ++i) {
        if (checkSerpentFull(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "Serpent", 0.90, "Serpent full S-box"});
        }
    }

    for (size_t i = 0; i + 32 <= size; ++i) {
        if (checkTwofish(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "Twofish", 0.80, "Twofish P-box"});
        }
    }

    for (size_t i = 0; i + 64 <= size; ++i) {
        if (checkSeed(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "SEED", 0.90, "SEED KC constants"});
        }
    }

    for (size_t i = 0; i + 32 <= size; ++i) {
        if (checkLea(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "LEA", 0.90, "LEA Delta constant"});
        }
    }

    for (size_t i = 0; i + 128 <= size; ++i) {
        if (checkHight(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "HIGHT", 0.90, "HIGHT Delta constants"});
        }
    }

    for (size_t i = 0; i + 32 <= size; ++i) {
        if (checkSafer(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "SAFER", 0.80, "SAFER EXP table"});
        }
    }

    for (size_t i = 0; i + 64 <= size; ++i) {
        if (checkCast128(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "CAST-128", 0.85, "CAST-128 S-box"});
        }
    }

    for (size_t i = 0; i + 32 <= size; ++i) {
        if (checkCast256(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "CAST-256", 0.85, "CAST-256 sigma table"});
        }
    }

    for (size_t i = 0; i + 64 <= size; ++i) {
        if (checkMars(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "MARS", 0.85, "IBM MARS S-box"});
        }
    }

    for (size_t i = 0; i + 64 <= size; ++i) {
        if (checkRijndaelTe(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "Rijndael-Te", 0.90, "Rijndael Te0 table"});
        }
    }

    for (size_t i = 0; i + 256 <= size; ++i) {
        if (checkSkipjack(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "Skipjack", 0.95, "Skipjack F-table"});
        }
    }

    for (size_t i = 0; i + 32 <= size; ++i) {
        if (checkSquare(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "Square", 0.80, "Square S-box"});
        }
    }

    for (size_t i = 0; i + 32 <= size; ++i) {
        if (checkShark(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "SHARK", 0.80, "SHARK S-box"});
        }
    }

    for (size_t i = 0; i + 8 <= size; ++i) {
        if (checkSimon(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "SIMON", 0.85, "SIMON-64/128 z3 constant"});
        }
    }

    for (size_t i = 0; i + 256 <= size; ++i) {
        if (checkShacal2(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "SHACAL2", 0.95, "SHACAL2 K constants"});
        }
    }

    for (size_t i = 0; i + 20 <= size; ++i) {
        if (checkSha1(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "SHA-1", 0.90, "SHA-1 H0 initial hash values"});
        }
    }

    for (size_t i = 0; i + 32 <= size; ++i) {
        if (checkSha256(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "SHA-256", 0.90, "SHA-256 H0 constants"});
        }
    }

    for (size_t i = 0; i + 48 <= size; ++i) {
        if (checkSha384(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "SHA-384", 0.90, "SHA-384 H0 initial hash values"});
        }
    }

    for (size_t i = 0; i + 64 <= size; ++i) {
        if (checkSha512(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "SHA-512", 0.90, "SHA-512 H0 initial hash values"});
        }
    }

    for (size_t i = 0; i + 192 <= size; ++i) {
        if (checkKeccak(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "Keccak/SHA-3", 0.95, "Keccak round constants"});
        }
    }

    for (size_t i = 0; i + 256 <= size; ++i) {
        if (checkMd2(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "MD2", 0.95, "MD2 PI_SUBST S-box"});
        }
    }

    for (size_t i = 0; i + 64 <= size; ++i) {
        if (checkMd4(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "MD4", 0.85, "MD4 T constants"});
        }
    }

    for (size_t i = 0; i + 48 <= size; ++i) {
        if (checkMd5mac(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "MD5MAC", 0.90, "MD5MAC T constants"});
        }
    }

    for (size_t i = 0; i + 20 <= size; ++i) {
        if (checkRipemd160(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "RIPEMD-160", 0.90, "RIPEMD-160 H0 initial values"});
        }
    }

    for (size_t i = 0; i + 32 <= size; ++i) {
        if (checkWhirlpool(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "Whirlpool", 0.85, "Whirlpool S-box"});
        }
    }

    for (size_t i = 0; i + 24 <= size; ++i) {
        if (checkTiger(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "Tiger", 0.90, "Tiger IV"});
        }
    }

    for (size_t i = 0; i + 32 <= size; ++i) {
        if (checkHaval(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "HAVAL", 0.85, "HAVAL IV"});
        }
    }

    for (size_t i = 0; i + 32 <= size; ++i) {
        if (checkBlake2s(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "BLAKE2s", 0.85, "BLAKE2s IV"});
        }
    }

    for (size_t i = 0; i + 192 <= size; ++i) {
        if (checkBlake2b(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "BLAKE2b", 0.95, "BLAKE2b SIGMA permutation"});
        }
    }

    for (size_t i = 0; i + 16 <= size; ++i) {
        if (checkChacha(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "ChaCha/Salsa20", 0.90, "ChaCha/Salsa20 constants"});
        }
    }

    for (size_t i = 0; i + 32 <= size; ++i) {
        if (checkDonna(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "Curve25519", 0.85, "Curve25519 field prime"});
        }
    }

    for (size_t i = 0; i + 40 <= size; ++i) {
        if (checkModm(data, size, i)) {
            hits.push_back({static_cast<uintptr_t>(i), "Curve25519", 0.85, "modm modular multiplication"});
        }
    }

    return hits;
}

std::vector<CryptoHit> FindCrypt3Engine::scanFile(const std::string& filePath) const {
    std::ifstream f(filePath, std::ios::binary | std::ios::ate);
    if (!f.is_open()) return {};

    size_t size = f.tellg();
    f.seekg(0);

    std::vector<uint8_t> data(size);
    f.read(reinterpret_cast<char*>(data.data()), size);

    return scanRegion(data.data(), size);
}

std::vector<std::string> FindCrypt3Engine::getSupportedAlgorithms() const {
    return {
        "AES", "DES", "SHA-256", "MD5", "Blowfish",
        "XXTEA", "RC4", "Whirlpool", "RIPEMD-160", "Camellia",
        "Serpent", "Twofish", "GOST", "RC2", "ChaCha/Salsa20",
        "SHA-1", "SHA-384", "SHA-512", "MD2", "MD4",
        "SEED", "LEA", "Tiger", "HAVAL", "BLAKE2s",
        "Keccak/SHA-3", "SIMON", "Skipjack", "Square", "SHARK", "Curve25519",
        "MD5MAC", "SHACAL2", "WAKE", "Curve25519-modm",
        "HIGHT", "SAFER", "CAST-256", "BLAKE2b",
        "CAST-128", "MARS", "Rijndael-Te", "RawDES-Spbox"
    };
}

bool FindCrypt3Engine::checkAesSbox(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 256 > size) return false;
    return memcmp(data + offset, AES_SBOX, 256) == 0;
}

bool FindCrypt3Engine::checkDesSbox(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 64 > size) return false;
    return memcmp(data + offset, DES_SBOX1, 64) == 0;
}

bool FindCrypt3Engine::checkSha256(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 32 > size) return false;
    return memcmp(data + offset, SHA256_H0, 32) == 0;
}

bool FindCrypt3Engine::checkMd5(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 256 > size) return false;
    // Check for MD5 T constants (first 16 as bytes)
    for (int i = 0; i < 16; ++i) {
        uint32_t val = *reinterpret_cast<const uint32_t*>(data + offset + i * 4);
        if (val != MD5_T[i]) return false;
    }
    return true;
}

bool FindCrypt3Engine::checkBlowfish(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 72 > size) return false;
    // Blowfish P-box starts with pi digits: 0x243f6a88, 0x85a308d3
    uint32_t p0 = *reinterpret_cast<const uint32_t*>(data + offset);
    uint32_t p1 = *reinterpret_cast<const uint32_t*>(data + offset + 4);
    return (p0 == 0x243f6a88 && p1 == 0x85a308d3);
}

bool FindCrypt3Engine::checkXxtea(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 4 > size) return false;
    uint32_t val = *reinterpret_cast<const uint32_t*>(data + offset);
    return (val == XXTEA_DELTA || val == XXTEA_DELTA_ALT);
}

bool FindCrypt3Engine::checkRc4Ksa(const uint8_t* data, size_t size, size_t offset) const {
    if (memcmp(data + offset, RC4_PAT_1, sizeof(RC4_PAT_1)) == 0) return true;
    if (memcmp(data + offset, RC4_PAT_2, sizeof(RC4_PAT_2)) == 0) return true;
    if (memcmp(data + offset, RC4_PAT_3, sizeof(RC4_PAT_3)) == 0) return true;
    if (memcmp(data + offset, RC4_PAT_4, sizeof(RC4_PAT_4)) == 0) return true;
    if (memcmp(data + offset, RC4_PAT_5, sizeof(RC4_PAT_5)) == 0) return true;
    if (memcmp(data + offset, RC4_PAT_6, sizeof(RC4_PAT_6)) == 0) return true;
    if (memcmp(data + offset, RC4_PAT_7, sizeof(RC4_PAT_7)) == 0) return true;
    return false;
}

bool FindCrypt3Engine::checkWhirlpool(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 32 > size) return false;
    return memcmp(data + offset, WHIRLPOOL_SBOX, 32) == 0;
}

bool FindCrypt3Engine::checkRipemd160(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 20 > size) return false;
    return memcmp(data + offset, RIPEMD160_H0, 20) == 0;
}

bool FindCrypt3Engine::checkCamellia(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 32 > size) return false;
    return memcmp(data + offset, CAMELLIA_SBOX, 32) == 0;
}

bool FindCrypt3Engine::checkSerpent(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 32 > size) return false;
    return memcmp(data + offset, SERPENT_SBOX, 32) == 0;
}

bool FindCrypt3Engine::checkTwofish(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 32 > size) return false;
    return memcmp(data + offset, TWOFISH_PBOX, 32) == 0;
}

bool FindCrypt3Engine::checkGost(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 16 > size) return false;
    return memcmp(data + offset, GOST_SBOX, 16) == 0;
}

bool FindCrypt3Engine::checkRc2(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 16 > size) return false;
    return memcmp(data + offset, RC2_SBOX, 16) == 0;
}

bool FindCrypt3Engine::checkChacha(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 16 > size) return false;
    return memcmp(data + offset, CHACHA_CONSTANTS, 16) == 0;
}

bool FindCrypt3Engine::checkSha1(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 20 > size) return false;
    return memcmp(data + offset, SHA1_H0, 20) == 0;
}

bool FindCrypt3Engine::checkSha384(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 48 > size) return false;
    return memcmp(data + offset, SHA384_H0, 48) == 0;
}

bool FindCrypt3Engine::checkSha512(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 64 > size) return false;
    return memcmp(data + offset, SHA512_H0, 64) == 0;
}

bool FindCrypt3Engine::checkMd2(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 256 > size) return false;
    return memcmp(data + offset, MD2_PI_SUBST, 256) == 0;
}

bool FindCrypt3Engine::checkMd4(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 64 > size) return false;
    return memcmp(data + offset, MD4_T16, 64) == 0;
}

bool FindCrypt3Engine::checkSeed(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 64 > size) return false;
    return memcmp(data + offset, SEED_KC, 64) == 0;
}

bool FindCrypt3Engine::checkLea(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 32 > size) return false;
    return memcmp(data + offset, LEA_DELTA, 32) == 0;
}

bool FindCrypt3Engine::checkSerpentFull(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 128 > size) return false;
    return memcmp(data + offset, SERPENT_SBOX_FULL, 128) == 0;
}

bool FindCrypt3Engine::checkTiger(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 24 > size) return false;
    return memcmp(data + offset, TIGER_IV, 24) == 0;
}

bool FindCrypt3Engine::checkHaval(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 32 > size) return false;
    return memcmp(data + offset, HAVAL_IV, 32) == 0;
}

bool FindCrypt3Engine::checkBlake2s(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 32 > size) return false;
    return memcmp(data + offset, BLAKE2S_IV, 32) == 0;
}

bool FindCrypt3Engine::checkKeccak(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 192 > size) return false;
    return memcmp(data + offset, KECCAK_RC, 192) == 0;
}

bool FindCrypt3Engine::checkSimon(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 8 > size) return false;
    return memcmp(data + offset, SIMON64_128_Z3, 8) == 0;
}

bool FindCrypt3Engine::checkSkipjack(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 256 > size) return false;
    return memcmp(data + offset, SKIPJACK_FTABLE, 256) == 0;
}

bool FindCrypt3Engine::checkSquare(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 32 > size) return false;
    return memcmp(data + offset, SQUARE_SBOX_PREFIX, 32) == 0;
}

bool FindCrypt3Engine::checkShark(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 32 > size) return false;
    return memcmp(data + offset, SHARK_SBOX_PREFIX, 32) == 0;
}

bool FindCrypt3Engine::checkDonna(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 32 > size) return false;
    return memcmp(data + offset, DONNA_FIELD_PRIME, 32) == 0;
}

bool FindCrypt3Engine::checkMd5mac(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 48 > size) return false;
    return memcmp(data + offset, MD5MAC_T, 48) == 0;
}

bool FindCrypt3Engine::checkShacal2(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 256 > size) return false;
    return memcmp(data + offset, SHACAL2_K, 256) == 0;
}

bool FindCrypt3Engine::checkWake(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 40 > size) return false;
    return memcmp(data + offset, WAKE_TT, 40) == 0;
}

bool FindCrypt3Engine::checkModm(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 40 > size) return false;
    return memcmp(data + offset, MODM_M_64, 40) == 0;
}

bool FindCrypt3Engine::checkHight(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 128 > size) return false;
    return memcmp(data + offset, HIGHT_DELTA, 128) == 0;
}

bool FindCrypt3Engine::checkSafer(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 32 > size) return false;
    return memcmp(data + offset, SAFER_EXP_PREFIX, 32) == 0;
}

bool FindCrypt3Engine::checkCast256(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 32 > size) return false;
    return memcmp(data + offset, CAST256_SIGMA_PREFIX, 32) == 0;
}

bool FindCrypt3Engine::checkBlake2b(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 192 > size) return false;
    return memcmp(data + offset, BLAKE2B_SIGMA, 192) == 0;
}

bool FindCrypt3Engine::checkCast128(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 64 > size) return false;
    return memcmp(data + offset, CAST128_S_PREFIX, 64) == 0;
}

bool FindCrypt3Engine::checkMars(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 64 > size) return false;
    return memcmp(data + offset, MARS_SBOX_PREFIX, 64) == 0;
}

bool FindCrypt3Engine::checkRijndaelTe(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 64 > size) return false;
    return memcmp(data + offset, RIJNDAEL_TE0_PREFIX, 64) == 0;
}

bool FindCrypt3Engine::checkRawDesSpbox(const uint8_t* data, size_t size, size_t offset) const {
    if (offset + 64 > size) return false;
    return memcmp(data + offset, RAWDES_SPBOX_PREFIX, 64) == 0;
}

std::vector<CryptoHit> FindCrypt3Engine::scanInstructions(
    const std::vector<SimpleInstruction>& instrs) const {
    std::vector<CryptoHit> hits;

    for (size_t i = 0; i < instrs.size(); ++i) {
        const auto& instr = instrs[i];

        if (instr.mnemonic == "eor" || instr.mnemonic == "xor") {
            hits.push_back({instr.address, "XOR", 0.8,
                "XOR obfuscation (eor/xor instruction)"});
        }

        if (instr.mnemonic == "ldrb" || instr.mnemonic == "ldrh" ||
            instr.mnemonic == "ldr") {
            for (size_t j = i + 1; j < instrs.size(); ++j) {
                if (instrs[j].address <= instr.address) continue;
                if (instrs[j].mnemonic == "eor" ||
                    instrs[j].mnemonic == "xor") {
                    hits.push_back({instr.address, "StringDec", 0.9,
                        "String decryption (ldr + eor pattern)"});
                }
                break;
            }
        }
    }

    return hits;
}

}  // namespace omnibyte::deob
